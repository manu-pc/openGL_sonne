...
void dibujarOrbita(Planeta *p)
{
	//? traza a órbita do planeta
	glBegin(GL_LINE_LOOP);
	glColor3f(p->color[0]/2, p->color[1]/2, p->color[2]/2);
	for (int i = 0; i < 360; i++)
	{
		// é preciso calcular a posición que tomará o planeta en cada ángulo
		float rad = i * PI / 180.0f;
		float xOrb = p->centroOrbitaX + p->semiejeMayor * cos(rad);
		float zOrb = p->centroOrbitaZ + p->semiejeMenor * sin(rad);

		float inclinacionRad = p->inclinacionOrbita * PI / 180.0f;
		float xOrbInclin = xOrb * cos(inclinacionRad);
		float yOrbInclin = xOrb * sin(inclinacionRad);
		float zOrbInclin = zOrb;

		if (p->orbitaAlrededor)
		{
			float px, py, pz;
			obtenerPosicionPlaneta(p->orbitaAlrededor, &px, &py, &pz);
			xOrbInclin += px;
			yOrbInclin += py;
			zOrbInclin += pz;
		}

		glVertex3f(xOrbInclin, yOrbInclin, zOrbInclin);
	}
	glEnd();
}
...
{
	//? obtén a posición absoluta dun planeta, empregada para o telescopio
	float anguloRad = p->orbitaActual * PI / 180.0f;

	// Posición en la órbita antes de aplicar la inclinación
	float posX = p->centroOrbitaX + p->semiejeMayor * cos(anguloRad);
	float posZ = p->centroOrbitaZ + p->semiejeMenor * sin(anguloRad);

	// Convertir la inclinación de grados a radianes
	float inclinacionRad = p->inclinacionOrbita * PI / 180.0f;

	// Rotar para inclinar la órbita
	float posXInclin = posX * cos(inclinacionRad);
	float posYInclin = posX * sin(inclinacionRad);
	float posZInclin = posZ; // La inclinación solo afecta X e Y

	// Si orbita otro planeta, sumamos su posición absoluta
	if (p->orbitaAlrededor)
	{
		float px, py, pz;
		obtenerPosicionPlaneta(p->orbitaAlrededor, &px, &py, &pz);
...
{
	float x, y, z;
	obtenerPosicionPlaneta(p, &x, &y, &z);
	//! esta función obtén a posición actual absoluta do planeta ou satélite, a partir dos seus atributos

	if (p->orbitaActiva)
		dibujarOrbita(p);

	// primeiro a traslación:
	glPushMatrix();
	glTranslatef(x, y, z); 
	glPushMatrix();

	// a inclinación da órbita visualízase xirando o planeta no eixo X
	glRotatef(p->inclinacionOrbita, 1.0f, 0.0f, 0.0f);

	// rotación sobre si mesmo:
	glRotatef(p->giroActual * p->velocidadGiro, 0.0f, 1.0f, 0.0f);

	// escalado a partir do radio:
	glScalef(p->radio, p->radio, p->radio);
	glColor3f(p->color[0], p->color[1], p->color[2]);

	// dous pop para manter a lóxica do main
	glCallList(p->lista);
	glPopMatrix();
	glPopMatrix();
}
